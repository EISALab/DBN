...
*  predict_ptron.cpp
*
*  function defining creation and use of perceptron models
*
*  03/05/2006	djh	created
*
***********************************************************************************/
...
int PredictPtron(int argc, char **argv, ostream& errMsg){
  ...
  if( mode == -1 || mode == 0 || mode == 2 )
  {
    ...
    Results_Train = model.Test( TrainExamples, t_val );
    //
...
//    ofile.close();
  ...
  if ( mode == 1 || mode == 2 || mode == 3)
  {
    ...
    Results_Test = model.Test( TestExamples, t_val );
    //
    // if using normalized values, unnormalize the results
    if( norm == 1 )
    {
