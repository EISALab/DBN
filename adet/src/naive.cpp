 ...
 *
 *
 * 03.06.2006	djh	eliminated reassignment of _ErrMean
 ...
 * 03.08.2006	djh	replaced _Upper/_LowerConfBound with _BoundStub
 * 04.21.2006	djh	removed nvar tgtIdx delay and nlags - replaced with predIdx
 *****************************************************************************************/
int naive_predictor::k_FoldXV( const int predIdx, const vector< vector< float > >& Ex ){
  // number of folds
  int nFolds = 10;
 ...
 * 04.21.2006	djh	removed nvar tgtIdx delay and nlags - replaced with predIdx
 ***********************************************************************/
void naive_predictor::Train( const int predIdx, const vector< vector< float > >& TrainEx )
{
  cout << "# Training:\n";
...
//    _pIdx=prevIdx;
  ...
  _BoundStub = sqrt( errVar * (1.0+( 1.0/float(TargPred.size()) )) );
}
...
/**********************************************************************
 * vector< float >  naive_predictor::TestHelper( const vector< float >& Ex, const float& t_val ) const
 * purpose:
 *   evaluates adet output for input pattern
 * param:
 *   Ex - pattern vector - target is first element
 *   t_val: T-test value 1.96 for 95%
 * return:
 ...
 **********************************************************************/
  vector< float > naive_predictor::TestHelper( const vector< float >& Ex, const float& t_val ) const{
    ...
    returnVal[0] = Ex[0];
    returnVal[1] = Ex[_pIdx]-_ErrMean-t_val*_BoundStub; //_UpperConfBound;
    returnVal[2] = Ex[_pIdx]-_ErrMean;
    returnVal[3] = Ex[_pIdx]-_ErrMean+t_val*_BoundStub; //_LowerConfBound;
...
/**********************************************************************
 *vector< vector< float > > naive_predictor::Test( const vector< vector< float > >& TestEx, const float& t_val ) const
 * input:
 *   TestEx: examples
 *   t_val: T-test value 1.96 for 95%
 *
 ...
 **********************************************************************/
 vector< vector< float > > naive_predictor::Test( const vector< vector< float > >& TestEx, const float& t_val ) const{
   ...
   float sum_xx = 0.0;
   for( int i=0; i< TargPred.size(); i++ ){
     TargPred[i] = TestHelper( TestEx[i], t_val);
