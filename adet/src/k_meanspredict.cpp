 ...
 *  stream: input stream
 *
 * 03.06.2006	djh	changed _totalUpper/_totalLowerConfBound to _totalBoundStub
 ******************************************************************************************/ 
 K_MeansPredict::K_MeansPredict( istream& stream ){
   int num_att;   
   stream >> _k >> num_att;
 ...
 *	(yes/no) is in prediction band	
 *****************************************************************************************/ 
 vector< vector< float > > K_MeansPredict::Test( const int PItype, const vector< vector< float > >& Examples, const float& t_val) const {
  cout << "# Testing:\n";
  ...
  for( int i=0; i<Examples.size(); i++ ){
    ...
    Coord< float > newCoord( temp );
    vector< float > predVal = PredictKey( PItype, newCoord, t_val );
