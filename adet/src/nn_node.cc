 ...
 **********************************************************************/
 nn_node::nn_node(const int n){
   ...
   // initialize weights to random double in range -1/sqrt(n)->1/sqrt(n) See Duda, Hart, and Stork p 311
   double w_bound = 1./sqrt(double(n));
   RNG r;
   for( int i=0; i< _w.size(); i++ ){
     //_w[i]=rando();
     //_w[i]=r.uniform(-1.,1.);
     //_w[i]=r.uniform(-0.5,0.5);
     //_w[i]=r.uniform(0.,0.5);
     _w[i]=r.uniform( -1.*w_bound, w_bound);
     //cout << _w[i] << endl;
   }
   // initialize bias to randon double in range -1/sqrt(n)->1/sqrt(n) See Duda, Hart, and Stork p 311
   //_bias = rando();
   //_bias=r.uniform(-1.,1.);
   //_bias=r.uniform(-0.5,0.5);
   //_bias=r.uniform(0.,0.5);
   _bias=r.uniform(-1.*w_bound, w_bound);
   //cout << _bias << endl;;
 }
