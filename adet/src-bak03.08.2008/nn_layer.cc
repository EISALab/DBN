 ...
 **********************************************************************/
 vector< double > nn_layer::BackpropError( const vector<double>& in, const vector<double>& err ){
   if( in.size() != NumInputs() ){
     cout<< "ERROR: vector< double > nn_layer::BackpropError( const vector<double>& in, const vector<double>& err )\n";
     cout <<" in.size!=NumInputs() \n";
     exit(-1);
   }
   else if( err.size() != NumNodes() ){
     cout<< "ERROR: vector< double > nn_layer::BackpropError( const vector<double>& in, const vector<double>& err )\n";
     cout <<" err.size() != NumNodes() \n";
     exit(-1) ;  
   }
   vector< double > returnVal( NumInputs(), 0. );
   for( int i=0; i < _nodes.size(); i++ ){
 ...
 * return:
 **********************************************************************/ 
 void nn_layer::UpdateLayer( const double& lr, const double& eta ){
   for( int i=0; i< _nodes.size(); i++ ){
     _nodes[i]->UpdateWeights( lr, eta );
   }
 }
