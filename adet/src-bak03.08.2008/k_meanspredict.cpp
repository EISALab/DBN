 ...
 **********************************************************/ 
 K_MeansPredict::K_MeansPredict( const string& fname ){
  ...
  if( !ifile ){
    cerr << "ERROR: K_MeansPredict::K_MeansPredict( const string& fname ) cannot open file " << fname << endl;
    exit( -1 );
  }
  *this = K_MeansPredict( ifile );
  ifile.close();
 }
 ...
 *****************************************************************************************/
 vector< float > K_MeansPredict::TestHelper( const vector< float >& Ex ) const{
   ...
   vector< float > predVal = PredictKey( 1, newCoord );
   returnVal[0] = Ex[0];
 ...
 * 
 * 02.18.2006 added isnan and isinf flags in _upper and lower pi prints
 *****************************************************************************************/ 
 void K_MeansPredict::Output( ostream& stream ) const
 {
   stream << setw(15) << _k << setw(15) << Dimension() << endl;
   ...
   for( int i=0;i<_upperConfBound.size(); i++ )
   {
     if( isinf(_upperConfBound[i]) && isnan(_upperConfBound[i]) ) stream << setw(15) << 0.0;
   ...
   for( int i=0;i<_lowerConfBound.size(); i++ )
   {
     if( isinf(_lowerConfBound[i]) && isnan(_lowerConfBound[i]) ) stream << setw(15) << 0.0;
...
// calculates alpha = 0.025 t-distribution for n examples
{
  ...
  else
  {
    ...
    //exit(-1);
    cout << " Warning:  error invalid number of degrees of freedom\n";
    return(100.);
  }
}
